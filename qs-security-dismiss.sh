#!/usr/bin/env bash
# Marks a finding (AUR-Malware or persistence) as reviewed/dismissed, or
# reactivates it. Called from SecurityWidget.qml (detail tab). The optional
# 4th argument says which status-file block to recompute ("aur_malware" by
...
NAME="$2"
DETAIL="$3"
BLOCK_KEY="${4:-aur_malware}"   # aur_malware | persistence
...
python3 -c '
import json, sys, hashlib, datetime, os, re

action, name, detail, dismissed_path, status_path, block_key = sys.argv[1:7]
...
# Recompute the affected block status/summary on the existing status file
...
block = status.get(block_key)
if not block or "issues" not in block:
    sys.exit()
...
active_fail = sum(1 for i in block["issues"] if i["status"] == "FAIL" and not i["dismissed"])
active_warn = sum(1 for i in block["issues"] if i["status"] == "WARN" and not i["dismissed"])
dismissed_n = sum(1 for i in block["issues"] if i["dismissed"])
block["status"] = "fail" if active_fail else ("warn" if active_warn else "clean")

if block_key == "persistence":
    active = active_fail + active_warn
    if active:
        summary = f"{active} persistence finding" + ("s" if active != 1 else "")
    else:
        summary = "no persistence indicators"
else:
    total = None
    m = re.search(r"out of (\d+) checks", block.get("summary", ""))
...
' "$ACTION" "$NAME" "$DETAIL" "$DISMISSED_FILE" "$STATUS_FILE" "$BLOCK_KEY"
